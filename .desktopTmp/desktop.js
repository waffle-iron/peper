...
class Desktop {
    ...
    constructor({ log, skeletonApp, appSettings, eventsBus, modules, Module }) {
        ...
        desktop.on('closeApp', () => _electron.app.quit());
        ...
        _process2.default.on('uncaughtException', Desktop.uncaughtExceptionHandler);
    ...
    static windowCrashedHandler() {
        Desktop.displayRestartDialog('Application has crashed', 'Do you want to restart it?');
    }
    ...
    static windowUnresponsiveHandler() {
        Desktop.displayRestartDialog('Application is not responding', 'Do you want to restart it?');
    }
    ...
    static uncaughtExceptionHandler(error) {
        ...
        // right?
        Desktop.displayRestartDialog('Application encountered an error', 'Do you want to restart it?', error.message);
    }
    ...
    static displayRestartDialog(title, message, details = '') {
        _electron.dialog.showMessageBox({ type: 'error', buttons: ['Restart', 'Shutdown'], title, message, detail: details }, response => {
            if (response === 0) {
